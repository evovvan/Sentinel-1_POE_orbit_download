...
####################################################################################
# Part of the "GAMMA Interferomety Scripts for Sentinel-1 (GISS)"        
#
# 1) the work path should have a text file containing the RAW data list,
#    e.g., raw_list.txt,
...
#    A folder with name of "POE" will be created in the work path for putting the downloaded EOF files
#
# Notes: Python3 & "axel" should be pre-installed 
# Install "axel" for Ubuntu OS: apt-get install axel 
##################################
import numpy as np
import datetime
...
#    im_list=os.popen('ls $RAW_path | grep ^S1.*SAFE$').read().split()
    ...
    for im_var in im_list:
        ...
        Inset_Day=format(Sence_day-datetime.timedelta(days=1)); #Determine the identifer day
##       
